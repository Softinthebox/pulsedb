<?php 
/**
    2017 P U L S E   F R A M E W O R K
*
* NOTICE OF LICENSE
*
...
* to license@pulseframework.com so we can send you a copy immediately.
*
* DISCLAIMER
*
...
*
*  @author Pedro Teixeira - Pulse Framework <dev@pulseframework.com>
*  @copyright  2017 Pulse Framework
*  @license    http://www.pulseframework.com/license
  International Registered Trademark & Property of Pulse Framework
*/
  
  
if (file_exists(_PATH_BIN_DIR_.'/settings/settings.inc.php'))
    require_once(_PATH_BIN_DIR_.'/settings/settings.inc.php');

class ErrorDB{
    // store errors
    private $errors = array();
    
    public $logs_path = _PATH_LOGS_DIR_;
    public $logExtension='.log';
    public $logExtensionError='.err';
    ...
    protected function make_log($content, $error=false){
        $fileName=date('G-i-s_d-m-Y');
        if($error){
            $fp = fopen($this->logs_path.$fileName.$this->logExtensionError, 'a');
            fwrite($fp, $content);
            fclose($fp);
        }else{
            $fp = fopen($this->logs_path.$fileName.$this->logExtension, 'a');
            fwrite($fp, $content);
            fclose($fp);
        }
    }
...
class DB extends ErrorDB{
    ...
    function __construct(){
        try {
            ...
            $this->db = new PDO($dsn, _DB_USER_, _DB_PASSWD_);
            $this->db->setAttribute(PDO::ATTR_EMULATE_PREPARES, false);
            $this->db->query('SET NAMES '.$this->charset);
        } catch (PDOException $e) {
            $this->make_log($e->getMessage(),true);
            die();
        }
    ...
    public function getValue($query){
        ...
        if($this->info->running==0){
            ...
            $this->info->vars=array('$query'=>$query);
        }
    ...
    public function get_row($query){
        ...
        if($pass){
            ...
            return $this->prep_vars(isset($result[0])?$result[0]:false );
    ...
    // general query function
    public function execute($query,$vals=''){
        ...
        if($sth){
            ...
            $this->info->running=0;
             $this->debug = true;
            $this->debug_formatted = true;
            $this->get_sql_error($this->db->errorInfo(),'Error executing query');
             //$this->debug = false;
            //$this->debug_formatted = false;
            return false;
        }
    ...
    public function query($query,$vals=''){
        ...
        if($query_from_cache = Cache::getQueryFromCache($query, $vals)){
            return $this->prep_vars($query_from_cache);
        }
        ...
        if($pass){
            ...
            Cache::setQueryCache($query, $vals, $result);
            return $this->prep_vars($result);
        }else{
        	$this->debug = true;
            $this->debug_formatted = true;
    ...
    public function exists($table,$where=false){
        $this->info->running=1;
    ...
    public function select_one($table,$vals='*',$where=false,$extra=''){
        $this->info->running=1;
    ...
    // insert
    public function insert($table,$vals,$extra=''){
        Cache::queryCacheRefresh();
        $this->info->running=1;
    ...
    public function insert_ignore($table,$vals){
    ...
    public function get_value($table,$val,$where=false){
        $this->info->running=1;
        $this->info->func='$db->get_value';
        $this->info->vars=array('$table'=>$table,'$val'=>$val,'$where'=>$where);
    ...
    private function _get_query($query,$val,$er=0){
        ...
        $html.= '</pre>';
        $html.= '<br /><strong>Db.class.php status:</strong><br /><pre>';
    ...
    public function _show_debug($title,$content,$pass=true){
        $html='';
